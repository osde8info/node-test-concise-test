...
export const toThrow = (fn, expected) => {
  try {
    ...
    if (actual.message !== expected.message)
      throw new Error(`Expected ${fn} to throw an exception, but the thrown error message did not match the expected message.\n` +
...
export const toHaveLength = (actual, expected) => {
  if (actual.length !== expected) {
    throw new Error(color(`Expected value to have length <bold>${expected}</bold> but it was <bold>${actual.length}</bold>`));
  }
};
